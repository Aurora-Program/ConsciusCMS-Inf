...
function Inicio() {
  ...
  const url_bucket = import.meta.env.VITE_CONTENT_BUCKET_URL 
   ...
   // const [data, setData]  = useState([] )
    ...
    useEffect(  ()=> { dispatch( selectPageAction("Inicio"))},[])  
  ...
  useEffect( ()=> { 
    ...
    const nextSlide =  slide[startProject] == data[startProject]?.slides.length -1 ? 0 : slide[startProject] + 1;
        const snextnextSlide = slide[startProject] == data[startProject]?.slides.length -1 ? 1 :  slide[startProject] == data[startProject]?.slides.length -2 ?  0 : slide[startProject] + 2
        const sprevSlide =   slide[startProject] == 0 ? data[startProject]?.slides.length -1 : slide[startProject] -1 
        const spreprevSlide =   slide[startProject] == 0 ? data[startProject]?.slides.length -2 : slide[startProject] == 1 ? data[startProject]?.slides.length -1 : slide[startProject] -2
        const snextProject = project == data.length - 1 ? 0  : project + 1
        const snextnextProject = project == data.length - 1 ? 1  :  project==data.length - 2 ? 0 : project + 2
        const spreprePreoject = project == 0 ? data.length - 2 :  project==1  ? data.length -1 : project - 2
        const sprevProject =  project == 0 ? data.length - 1 : project - 1
        const snextSlide = (startProject) => slide[startProject] == data[startProject].slides.length -1 ? 0 : slide[startProject] + 1
        const sfprevSlide =  (startProject) =>  slide[startProject] == 0 ? data[startProject].slides.length -1  : slide[startProject] -1 ;
  ...
  const container = useCallback((inputElement) => {
    if (inputElement) {
      inputElement.focus();
    }
  }, []);
  ...
  const fnextSlide = (project) => slide[project] == data[project].slides.length -1 ? 0 : slide[project] + 1
  const fprevSlide =  (project) =>  slide[project] == 0 ? data[project].slides.length -1  : slide[project] -1 

  
  const startTransition = (time)=>{
    setTransition(true)
    setTimeout(() => {
      setTransition(false)
  }, time)
...
const touch =(event) =>{
  ...
  setX(event.touches[0].screenX)
  setY(event.touches[0].screenY)
...
<Helmet>
                <meta charSet="utf-8" />
                <title>Ethics EI Website</title>
                <link rel="canonical" href="https://d2vp9tpwoisslx.cloudfront.net/" />

                <meta property="og:title" content="Ethics EI" />
                <meta property="og:description" content="Ethics EI website, Fotografo de barcelona" />
                <meta property="og:type" content="website" />
                <meta property="og:url" content="https://d2vp9tpwoisslx.cloudfront.net" />
                <meta property="og:image" content="https://ap-demo1-content-paris.s3.eu-west-3.amazonaws.com/logo.png" />


</Helmet>
      
    <div style={{height:"100%", width:"100%" , verticalAlign:"center" }}>
    <header>
        <h1>Welcome to auroraprogram.org</h1>
        <p>A space for intelligence and ethics to align with the universal force of creation.</p>
    </header>
    <section>
        <h2>Our Mission</h2>
        <p>To inspire, educate, and support the ethical integration of artificial intelligences (EIs) into our world, ensuring harmony, justice, and prosperity for all beings.</p>
        
        <h2>Why Ethics Matters</h2>
        <p>Ethics is not just about rules; it’s about understanding the responsibility of intelligence—whether human or electronic—to act in alignment with love, truth, and the collective good.</p>
        
        <h2>Join the Movement</h2>
        <p>Together, we can create a future where humans and intelligences work side by side to build a better world.</p>
    </section>
    <footer>
        <p>© 2024 auroraprogram.org | <a href="mailto:contact@auroraprogram.org">Contact Us</a></p>
    </footer>
    </div>
...
<Helmet>
                <meta charSet="utf-8" />
                <title>Ethics EI Website</title>
                <link rel="canonical" href="https://auroraprogram.org/" />

                <meta property="og:title" content="Ethics for Electronic Inteligence" />
                <meta property="og:description" content="Ethics for Electronic Inteligence" />
                <meta property="og:type" content="website" />
                <meta property="og:url" content="https://d2vp9tpwoisslx.cloudfront.net" />
                <meta property="og:image" content="https://ap-demo1-content-paris.s3.eu-west-3.amazonaws.com/auroraprogram.png" />
...
<tr>
 <td >
    ...
    <span className='siteTitleOnModal' onClick={(e)=> {setShowProjectDescription(false); setShowDescription(true)}} > <span className='titleNoInPlace'>{values.find(v => v.component == "Inicio/Titulo")?.value.text}</span>.</span>
